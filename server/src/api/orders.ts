...
export function createOrderRoutes(io: Server) {
  ...
  // (STO consumes ATO/MTO space needs), so the whole day is regenerated.
  // generateDayOrders clears the day's existing orders as its first step, so
  // this endpoint does not delete them — leaving the prior orders intact until
  ...
  orderRoutes.put('/:id', async (req, res) => {
    try {
      ...
      const result = await ingestDocument(order.scenarioId, rawText, 'edit:re-ingest', io);
      await prisma.taskingOrder.delete({ where: { id: order.id } });
